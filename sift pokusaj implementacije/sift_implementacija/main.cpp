/**********************************************************************************************************\
Ova implementacija je preuzeta sa:
https://github.com/Saleh-I/SIFT/blob/master/SIFT/main.cpp
Postoji više problema sa ovom implementacijom koje sam pokušao riješiti, a glavni je to da se nepravilno
identifikuju tačke od interesa, a za što nisam uspio pronaći adekvatno rješenje. Međutim, ovaj kod ipak
prilažem uz izvještaj, jer može poslužiti kao osnova za neki budući rad na SIFT-u, te doprinijeti njegovom
razumijevanju s obzirom da su sve funkcije potrebne SIFT detektor i SIFT deskriptor implementirane ručno
uz preporuke date u originalnom radu "Distinctive Image Features from Scale-Invariant Keypoints" autora
David Lowe-a.
***********************************************************************************************************/
...
int main() {
    ...
    Mat img, img2, scr, scr2, base_img, base_img2;

    img=imread("venecija.jpg", 0);
    img2 = imread("venecija2.jpg", 0);
    img.convertTo(scr, CV_32FC1);
    img2.convertTo(scr2, CV_32FC1);

    int num_intervals = 3; //3
    ...
    vector<Mat> key_descriptors = mysift.descriptor(keypoints, get<0>(Scale_DoG)); //promjena

    Mat image_with_keypoints;
    drawKeypoints(img, keypoints, image_with_keypoints);
    cout<<"Broj karakterisicnih tacaka za prvu sliku je: "<<keypoints.size()<<endl;
    //for (int i=0; i<keypoints.size();i++)
    //  cout<<"pronadjene su tacke: "<<keypoints[i].pt.x<<" "<<keypoints[i].pt.y<<endl; //dobivene koordinate nisu uredu
    imshow("Znacajke sa prve slike", image_with_keypoints);

    //isto sve za drugu sliku
    Sift_feature mysift2;
    base_img2 = mysift2.generate_base_img(scr2);

    vector<double> sigmas2 = mysift2.generate_sigmas(sigma, num_intervals);
    tuple<vector<vector<Mat>>, vector<vector<Mat>>> Scale_DoG2 = mysift2.Create_Scale_Space(base_img2, num_intervals, sigmas2);
    vector<KeyPoint> keypoints2 = mysift2.Extrema(get<1>(Scale_DoG2), get<0>(Scale_DoG2), num_intervals, sigma);
    vector<Mat> key_descriptors2 = mysift2.descriptor(keypoints2, get<0>(Scale_DoG2));

    Mat image_with_keypoints2;
    drawKeypoints(img2, keypoints2, image_with_keypoints2);
    imshow("Znacajke sa druge slike", image_with_keypoints2);

    //pretvorba vektor<Mat> gdje je Mat vektor kolona u Matricu gdje 1 red predstavlja deskriptor za jednu keypoint
    Mat desc(key_descriptors.size(),128,CV_32FC1);

    for(int i=0; i<key_descriptors.size();i++)
      for(int j=0; j<128; j++)
        desc.at<float>(i,j)=key_descriptors[i].at<float>(j,0); //vektor matrica koje su vektor kolona

    Mat desc2(key_descriptors2.size(),128,CV_32FC1);

    //pretvorba vektor<Mat> gdje je Mat vektor kolona u Matricu gdje 1 red predstavlja deskriptor za jednu keypoint
    for(int i=0; i<key_descriptors2.size();i++)
       for(int j=0; j<128; j++)
           desc2.at<float>(i,j)=key_descriptors2[i].at<float>(j,0); //vektor matrica koje su vektor kolona
    ...
    //pokusaj resize originalnih slika
    //resize(img, img, Size(0, 0), 2, 2, INTER_LINEAR);
    //resize(img2, img2, Size(0, 0), 2, 2, INTER_LINEAR);

    namedWindow("Matches", 1);
    Mat img_matches;
    drawMatches(img, keypoints, img2, keypoints2, matches, img_matches);
